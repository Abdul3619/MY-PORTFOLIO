...
const searchString = `No download logs recorded</div>`;
const idx = content.indexOf(searchString);
if (idx === -1) {
  console.log("Not found");
  process.exit(1);
}
const before = content.substring(0, idx + searchString.length);
const replacement = `
              )}
            </div>
          </div>
...
`;
fs.writeFileSync('src/pages/admin/AdminResume.tsx', before + replacement);
